  ...
  methods: {
    ...
    async valueChanged(e) {
      switch (e.target.dataset.name) {
        ...
        case "start_time":
          ...
          let now_time =
            new Date().getHours() * 3600 + new Date().getMinutes() * 60; //当日时间
          now_time *= 1000;
        ...
        case "end_time":
          if (this.data.date_start !== "请选择") {
            ...
            let end_date_temp = new Date(e.detail.value).setHours(0, 0, 0, 0);
            let now_date_temp = new Date().setHours(0, 0, 0, 0);
        ...
        case "frequency":
          ...
          this.setData({
            ...
            frequencynum: d,
            frequencyout:
              this.data.frequencyArray[0][0] +
              this.data.frequencyArray[1][f[1]] +
              this.data.frequencyArray[2][f[2]] +
              this.data.frequencyArray[3][f[3]],
          });
          break;
        ...
        case "needed_be_signed_deadline":
          ...
          let startTotalMinutes =
            parseInt(this.data.time_aim1.split(":")[0]) * 60 +
            parseInt(this.data.time_aim1.split(":")[1]);
          let endTotalMinutes =
            parseInt(e.detail.value.split(":")[0]) * 60 +
            parseInt(e.detail.value.split(":")[1]);
      ...
      if (
        e.target.dataset.name != "frequency" &&
        e.target.dataset.name != "reminder_at"
      ) {
    ...
    async btn_share(e) {
      ...
      if (
        ...
        ((this.$state.aimCardData["goal_type"] != 1 &&
          this.$state.aimCardData["goal_type"] != null) ||
          (this.$state.aimCardData["end_time"] != null &&
            this.$state.aimCardData["needed_be_signed_deadline"] != null))
      ) {
        this.setData({
        ...
        !this.$state.aimCardData["team"]
          ? (team = 0)
          : (team = this.$state.aimCardData["team"]);
        ...
        if (goal_type == 1) {
          ...
          let result = await awx.request({
            ...
            data: {
              ...
              needed_be_signed_at:
                this.$state.aimCardData["needed_be_signed_at"],
              needed_be_signed_deadline:
                this.$state.aimCardData["needed_be_signed_deadline"],
              signed_type: 1,
              signed_day: 0,
        ...
        let data = {
          ...
          name:
            goal_type == 2
              ? `每天走${this.$state.aimCardData["title"]}步`
              : this.$state.aimCardData["title"],
        };
